...
MCP 客户端（DSH / Claude Code / Codex / Cursor / VS Code / Claude Desktop）
  └─ 拉起 gitbash-mcp（Node 进程，沙箱外，stdio）
       ├─ 无参数        → server.js        → 注册 exec / bash_info / doctor / policy
...
├── server.js              ← MCP server：工具注册 + 裁决接线 + 服务端 instructions
├── lib/                   ← 共享逻辑（纯函数优先，便于无 TTY 单测）
│   ├── detect.js          ← bash 检测（惰性 + 缓存）+ doctor 报告
│   ├── runner.js          ← spawn bash + 截断/spill + 并发 + 杀树 + 环境洗白
│   ├── shell-parse.js     ← 命令解析：切段 / 去引号 / 标记 opaque（纯函数）
...
├── test/                  ← 五套测试（不入发布包）
│   ├── test-client.mjs    ← MCP 协议冒烟
│   ├── test-cli.mjs       ← CLI 冒烟（临时 root）
│   ├── test-menu.mjs      ← 菜单按键逻辑单测
│   ├── test-runner.mjs    ← P0 护栏单测
│   └── test-policy.mjs    ← 策略单测
└── docs/
...
| `runner.js` | `spawnBash` `collectStream` `createSemaphore` `killTree` `scrubEnv` | 常量：`DEFAULT_TIMEOUT_MS=60000` `MAX_TIMEOUT_MS=600000` `OUTPUT_CAP_BYTES=64KB` `SPILL_CAP_BYTES=64MB` `MAX_CONCURRENCY=4` |
| `shell-parse.js` | `parseCommand` | 纯函数；here-doc 正文跳过；`$( )`/反引号/`$'…'`/变量程序名/子 shell → `opaque` |
| `policy.js` | `evaluateCommand` `decide` `currentStance` `describePolicy` `projectEntry` `lists` | 档位：read-only / project / ask-required / dangerous / catastrophic；唯一开关 `GITBASH_MCP_RISKY` |
...
| 工具 | 作用 | 备注 |
|---|---|---|
| `exec` | 跑 bash 命令/多行脚本，返回统一 JSON | 描述里声明「Windows 上优先选它」；失败回 JSON，不抛工具错误 |
| `bash_info` | bash 路径 + bash/git 版本 | 缺 bash 时回修复说明 |
| `doctor` | 完整环境诊断 | bash 异常先调它 |
| `policy` | 打印当前姿态与完整规则 | 被 `APPROVAL_REQUIRED`/`POLICY_DENIED` 后调它向用户解释 |

服务端还通过 MCP `initialize` 的 `instructions` 字段声明「gitbash-mcp 是沙箱外的 git-bash，Windows 上优先用 `exec`」；支持的客户端会把它注入模型上下文。

## 6. 契约与常量速查

- **结果契约**（`exec`，见 `DESIGN.md` §4.1）：`exit_code / stdout / stderr / timed_out / truncated / spill_path / spill_bytes / spill_truncated / duration_ms / killed_by / audit_id / queued_ms`，外加 `policy` 与缺 bash 时的 `error_code` / `hint`。
- **唯一环境变量**：`GITBASH_MCP_RISKY=ask`（默认）/ `allow`，由人类在 MCP 客户端配置里注入，模型不得自行设置。
- **日志只走 stderr**：stdout 是 MCP 协议通道。
- **护栏零配置**：并发/封顶/洗白/审计都由 `lib/runner.js` 的常量决定。
- **发布白名单**：`package.json` 的 `files` = `bin/`、`lib/`、`server.js`、`README.md`、`LICENSE`（当前打 13 个文件）。
...
| 执行 / 超时 / 截断 / spill / 并发 / 洗白 | `lib/runner.js` | `test/test-runner.mjs` |
| 解析（切段 / 引号 / opaque） | `lib/shell-parse.js` | `test/test-policy.mjs` |
...
| 工具定义 / 描述 / 服务端 instructions | `server.js` | `test/test-client.mjs` |
...
| 套件 | 覆盖 | 能否在受限沙箱内跑 |
|---|---|---|
| `test/test-client.mjs` | 工具握手、doctor、管道、非零退出、超时整树杀、spill、取消、策略、缺 bash 降级 | 否（spawn bash + 管道） |
...
| `test/test-runner.mjs` | 洗白、spill 双重封顶、并发、取消杀树、审计往返与轮转 | 部分（杀树要 bash） |
| `test/test-policy.mjs` | 解析、43 个档位分类用例、项目信任、姿态裁决、报告 | 是（纯函数） |
