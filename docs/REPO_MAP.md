...
| 套件 | 覆盖 | 能否在受限沙箱内跑 |
|---|---|---|
| `test/test-client.mjs` | 工具握手、doctor、管道、非零退出、生命期超时杀树、后台作业全流程、取消移交、完成判定（`sleep 5 &` 立即返回）、路径转换（`cmd /c` 正常 / `cmd //c` 拒绝而非挂死 / env 逃生口）、spill、策略、缺 bash 降级 | 否（spawn bash + 管道） |
...
| `test/test-runner.mjs` | 洗白与 `MSYS_NO_PATHCONV`、spill 双重封顶、通道 tail/offset、并发、取消杀树、完成判定跟着 shell、预算移交、移交后时限归零、作业生命周期、审计往返与轮转 | 部分（杀树/作业要 bash） |
