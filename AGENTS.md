...
├── test/                 ← 冒烟与单测（不入发布包）
│   ├── test-client.mjs   ← 服务端协议冒烟（含后台作业 / 取消移交）
│   ├── test-cli.mjs      ← CLI 冒烟（临时 root，不碰真实配置）
│   ├── test-menu.mjs     ← 菜单按键逻辑单测（无需 TTY）
│   ├── test-runner.mjs   ← 护栏与执行原语单测：洗白 / 封顶 / 通道 / 并发 / 杀树 / 预算移交 / 作业 / 审计
│   └── test-policy.mjs   ← 策略单测：解析回归 / 档位分类 / 姿态裁决 / 报告
├── .gitignore
└── docs/
    ├── DESIGN.md
    ├── PLAN.md
...
| `node test/test-runner.mjs` | 护栏与执行原语单测（洗白 / 封顶 / 通道 / 并发 / 杀树 / 预算移交 / 作业 / 审计） |
...
- 共享逻辑放 `lib/`：`detect.js`（检测）、`runner.js`（子进程+护栏+输出通道）、`jobs.js`（后台作业注册表）、
  `shell-parse.js`（解析）、`policy.js`（策略）、`audit.js`（审计）、
