...
// How long the pipes may keep draining after the shell itself exited: enough for
// the last buffered bytes, without waiting for a leftover holder (DESIGN §5.16).
...
export function createChannel(options) {
  ...
  let spillFlush = null
  const finishSpill = () => {
    if (spillFlush !== null) return spillFlush
    const s = spillStream
    if (!s) {
      spillFlush = Promise.resolve()
      return spillFlush
    }
    spillFlush = new Promise((resolve) => {
      let settled = false
      const settle = () => {
        if (settled) return
        settled = true
  ...
  return {
    ...
    /** Consume a readable stream until it ends; resolves when the spill is flushed. */
    attach(stream) {
      source = stream
      return new Promise((resolve) => {
        onEnd = () => {
          if (ended) return
          ended = true
          finishSpill().then(() => resolve(snapshot()))
        }
    ...
    /**
     * Stop collecting because the command itself is done: detach, then resume so
     * a leftover holder of the pipe keeps draining instead of blocking on write.
     */
    stop() {
      if (source !== null && !ended) {
        ended = true
        source.removeListener('data', write)
        if (onEnd !== null) {
...
export function launch(bashPath, command, options) {
  ...
  const killNote = () => (killedBy === null ? '' : (killedBy === 'timeout'
    ? '[timeout after ' + lifetimeMs + 'ms]'
    : '[' + killedBy + ']'))
  // Kill the tree, then settle within a bounded grace period even when an
  // orphaned MSYS2 grandchild keeps the stdio pipes open (see settleWithExitCode).
  const killNow = (kind) => {
    if (killedBy) return
    killedBy = kind
  ...
  const exitResult = (code) => Object.assign(collect(), {
    exit_code: killedBy !== null ? -1 : (code === null || code === undefined ? -1 : code),
    stderr: stderr.snapshot().text + killNote(),
    timed_out: timedOut,
    duration_ms: Date.now() - startedAt,
    killed_by: killedBy,
  })
  // Completion follows the shell, not the pipes: `start`, `&` and daemons leave
  // grandchildren holding stdout long after the command is done (DESIGN §5.16).
  // Wait for the streams to end, then settle anyway after a short drain window.
  let exited = false
  let exitCode = null
  let streamsLeft = 2
  let drainTimer = null
  const settleWithExitCode = () => { finish(exitResult(exitCode)) }
  const armSettle = () => {
    if (settled || drainTimer !== null) return
    if (streamsLeft <= 0) { settleWithExitCode(); return }
    drainTimer = setTimeout(() => {
      Promise.all([stdout.stop(), stderr.stop()]).then(settleWithExitCode)
    }, EXIT_DRAIN_MS)
  }
  const onStreamGone = () => {
    streamsLeft -= 1
    if (streamsLeft <= 0 && exited) armSettle()
  }
  const onAbort = () => {
  ...
  const stdoutEnded = (child.stdout ? stdout.attach(child.stdout) : Promise.resolve()).then(onStreamGone)
  const stderrEnded = (child.stderr ? stderr.attach(child.stderr) : Promise.resolve()).then(onStreamGone)

  child.on('error', (err) => {
    finish(Object.assign(exitResult(-1), { stderr: stderr.snapshot().text + '[spawn error] ' + err.message }))
  })
  child.on('exit', (code) => {
    exited = true
    exitCode = code
