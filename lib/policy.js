...
function classifySegment(seg, ctx) {
  ...
  if (seg.program === '') return { kind: 'read-only', reason: 'shell assignment' }
  ...
  if (seg.redirections.some((r) => /^\/dev\/(sd|hd|nvme)/.test(r.target || ''))) return { kind: 'catastrophic', reason: 'writing to a raw device' }
  ...
  if (seg.redirections.some((r) => /(\.bashrc|\.bash_profile|\.profile$|authorized_keys|\.ssh[\/\\])/.test(r.target || ''))) return { kind: 'dangerous', reason: 'writing to a shell profile or ssh keys' }
  if (seg.redirections.length > 0) return { kind: 'mutating', reason: 'writes to ' + seg.redirections.map((r) => r.target).join(', ') }
...
export function evaluateCommand(command, options) {
  ...
  const reasons = []
  const kinds = []
  if (/:\s*\(\s*\)\s*\{/.test(String(command || ''))) { kinds.push('catastrophic'); reasons.push('fork bomb') }
